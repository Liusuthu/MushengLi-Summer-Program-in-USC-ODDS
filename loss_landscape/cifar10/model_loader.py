...
def my_load(model_name, model_file=None, data_parallel=False):
    num_classes=100
    net = timm.create_model(model_name,pretrained=True,num_classes=num_classes)
    ...
    # device= torch.device('cuda:0' if torch.cuda.is_available() else "cpu") #默认为cuda:0
    # net=net.to(device) 
    net.eval()
    return net
