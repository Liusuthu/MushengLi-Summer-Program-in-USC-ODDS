...
def main(args):
    def get_gpu_state():
        nvmlInit()  #初始化
        handle = nvml.nvmlDeviceGetHandleByIndex(int(args.device[5:])) #获取句柄
        temperature = nvmlDeviceGetTemperature(handle,NVML_TEMPERATURE_GPU)
        memory_info = nvmlDeviceGetMemoryInfo(handle)
        memory_usage=(memory_info.used/1e6)
        return (temperature,memory_usage)
    ...
    FILE_PATH="./GPU_result/"+EXPERIMENT_NAME+"/"
    ...
    save_json(GPU_log,FILE_PATH+"GPU.json")
    ...
    net = timm.create_model(args.model,pretrained=True,num_classes=int(num_classes))    
    net = ModuleValidator.fix(net); # fix使其能用于DP训练
    net=net.to(device) 
    # 2. attaching the model to the device.
    timepoint=time.time()-start_time
    GPU_log={
        'label':'After attaching the model to device',
        'time':timepoint,
        'GPU_state':get_gpu_state(),
    }
    save_json(GPU_log,FILE_PATH+"GPU.json")
    ...
    if 'nonDP' not in args.clipping_mode:
        ...
        # 3. After optimizer.attach(), for non-DP we don’t have this step.
        timepoint=time.time()-start_time
        GPU_log={
            'label':'After attaching the optimizer to PrivacyEngine',
            'time':timepoint,
            'GPU_state':get_gpu_state(),
        }
        save_json(GPU_log,FILE_PATH+"GPU.json")
    ...
    def train(epoch,privacy_engine):
        ...
        for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader)): #这里的batch_idx应该相当于step？
            ...
            if ((batch_idx + 1) % n_acc_steps == 0) or ((batch_idx + 1) == len(trainloader)):
                loss.backward()
                # moniter insert here (peak)
                if((batch_idx + 1) % SAVE_GPU_STEPS == 0) or ((batch_idx + 1) == len(trainloader)):
                    timepoint=time.time()-start_time
                    GPU_log={
                        'label':'loss_backward',
                        'time':timepoint,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mini_bs', type=int, default=200)
